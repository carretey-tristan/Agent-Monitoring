"""
Module de surveillance des disques
----------------------------------
Surveille l'espace disque (partitions) et les performances I/O (disques physiques).

Exposed Functions:
- get_data(): Retourne une structure complexe {disk_name: {stats...}}.
- get_influx_points(): Transforme les données brutes en Points InfluxDB (Mesures: disk & diskio).
"""

import psutil
from influxdb_client import Point

def get_influx_points(data, hostname, company):
    """
    Génère les points InfluxDB pour le module disque.
    """
    points = []
    
    if isinstance(data, dict) and "error" not in data:
        for disk_name, disk_content in data.items():
